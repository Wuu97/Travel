  ...
  return <>
    ...
    {open && <div className="auth-backdrop">
      <section className="auth-dialog" data-modal-scroll-lock role="dialog" aria-modal="true" aria-labelledby="auth-title">
        ...
        {registeredEmail ? <div className="auth-success-state"><div className="auth-success-icon" aria-hidden="true">✓</div><h2>注册成功</h2><p>验证邮件已发送至<br /><strong>{registeredEmail}</strong></p><p className="auth-success-help">请前往邮箱完成验证，验证后即可登录。</p>{(error || notice) && <p className={`auth-message${error ? " auth-message-error" : ""}`} role="status">{error || notice}</p>}<Button type="button" onClick={goToSignIn}>去登录</Button><Button disabled={submitting} size="sm" type="button" variant="link" onClick={() => void resendVerification()}>{submitting ? "正在发送…" : "没收到邮件？重新发送验证邮件"}</Button></div> : <form className="auth-dialog-form" noValidate onSubmit={(event) => { event.preventDefault(); void submit(); }}>
          ...
          {method === "phone" ? phoneOtpSent && <Button disabled={submitting} size="sm" type="button" variant="link" onClick={() => { onClearError(); setPhoneOtpSent(false); setVerificationCode(""); setNotice(null); }}>更换手机号</Button> : <Button disabled={submitting} size="sm" type="button" variant="link" onClick={() => { onClearError(); setMode(isSignUp ? "sign-in" : "sign-up"); setNotice(null); }}>{isSignUp ? "已有账户？去登录" : "没有账户？去注册"}</Button>}
        </form>}
      </section>
    </div>}
  </>;
