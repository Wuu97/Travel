...
export function CalendarPopover({ ariaLabel, disabled = () => false, inRange = () => false, month, onClose, onDateHover, onDateSelect, onMonthChange, onPointerLeave, selected = () => false }: Props) {
  ...
  return <div className="return-date-calendar" ref={rootRef} role="dialog" aria-label={ariaLabel}>
    <div className="calendar-toolbar"><button type="button" aria-label="上个月" onClick={() => moveMonth(-1)}>‹</button><div><button type="button" onClick={() => setView("years")}>{year} 年</button><button type="button" onClick={() => setView("months")}>{monthNumber} 月</button></div><button type="button" aria-label="下个月" onClick={() => moveMonth(1)}>›</button><button className="calendar-today" type="button" onClick={() => { onMonthChange(today.slice(0, 7)); setView("days"); }}>今天</button></div>
