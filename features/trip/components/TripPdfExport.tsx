...
type Props = { budgetItems: ExpenseItem[]; days: Array<{ day: number; date: string }>; details: TripDetails; expenses: LedgerItem[]; onClose: () => void; open: boolean; plans: ItineraryItem[] };

const money = (amount: number) => `¥${amount.toFixed(0)}`;

/** Preview-first PDF export that avoids the browser print dialog. */
export function TripPdfExport({ budgetItems, days, details, expenses, onClose, open, plans }: Props) {
  const previewRef = useRef<HTMLDivElement>(null);
  const [exporting, setExporting] = useState(false);
  const overview = getBudgetOverview(budgetItems, expenses);
  ...
  return <PdfPreviewDialog backLabel="返回编辑" exporting={exporting} onClose={onClose} onDownload={() => void download()} title="确认行程排版" titleId="pdf-preview-title"><div className="pdf-preview-pages" ref={previewRef}><div className="pdf-document">
      ...
      </div><div className="pdf-document pdf-budget"><header className="pdf-document-header"><p>TRIP BUDGET</p><h1>{details.title}</h1><span>费用概览与预算汇总</span></header><section><h2>账本</h2><div className="pdf-totals"><div><span>总预算</span><strong>{money(overview.plannedTotal)}</strong></div><div><span>已支出</span><strong>{money(actualTotal)}</strong></div><div><span>剩余可用</span><strong>{money(overview.plannedTotal - actualTotal)}</strong></div></div></section></div></div></PdfPreviewDialog>;
}
