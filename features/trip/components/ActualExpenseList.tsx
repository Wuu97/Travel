    ...
    <div className="expense-list" style={{ display: "flex", flexDirection: "column", height: "100%", margin: 0, minHeight: 0, overflowY: "auto" }}>
      ...
      {expenses.map((expense) => (
        <article key={expense.id}>
          ...
          <div><h4>{expense.item}</h4><p>{expense.type} · {expense.by} 支付{expense.relatedItineraryTitle ? ` · 关联 ${expense.relatedItineraryTitle}` : ""}</p></div>
          <strong>¥ {expense.amount}</strong><button type="button" onClick={() => onEdit(expense.id)}>编辑</button><button className="expense-remove" type="button" aria-label={`删除${expense.item}`} onClick={() => onRemove(expense.id)}>×</button>
        </article>
      ))}
    </div>
  );
