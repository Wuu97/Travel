...
export function CustomDateRangePicker({ endDate, onChange, startDate, allowClear = false, className = "", endError, endLabel = "返程", rangeClearVariant = "text", showIndividualClear = true, showLabels = true, showSeparator = false, startError, startLabel = "出发" }: Props) {
  ...
  return <div className={`custom-date-range ${className}`} data-calendar-target={target}>
    <div className="date-popover-fields"><label>{showLabels && startLabel}<span className="date-field-control"><input aria-label={startLabel} className={startError ? "input-error" : ""} placeholder={`选择${startLabel}`} readOnly value={startDate.replaceAll("-", "/")} onClick={() => openFor("start")} />{allowClear && showIndividualClear && startDate && <IconButton aria-label={`清除${startLabel}`} icon="clear" size="sm" variant="ghost" onClick={() => clear("start")} />}</span>{startError && <small className="field-error">{startError}</small>}</label>{showSeparator && <span aria-hidden="true" className="date-range-separator">-</span>}<label>{showLabels && endLabel}<span className="date-field-control"><input aria-label={endLabel} className={endError ? "input-error" : ""} placeholder={`选择${endLabel}`} readOnly value={endDate.replaceAll("-", "/")} onClick={() => openFor("end")} />{allowClear && showIndividualClear && endDate && <IconButton aria-label={`清除${endLabel}`} icon="clear" size="sm" variant="ghost" onClick={() => clear("end")} />}</span>{endError && <small className="field-error">{endError}</small>}</label>{allowClear && (startDate || endDate || rangeClearVariant === "icon") && (rangeClearVariant === "icon" ? <span className={`date-range-clear is-icon${startDate || endDate ? "" : " is-hidden"}`}><IconButton aria-label="清除日期筛选" icon="clear" size="sm" variant="ghost" onClick={clearRange} /></span> : <button aria-label="清除日期筛选" className="date-range-clear" type="button" onClick={clearRange}>清除日期</button>)}</div>
