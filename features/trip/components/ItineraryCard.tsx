...
export function ItineraryCard({ index, inlineEdit, inlineRef, isMenuOpen, menuRef, onChangeInline, onCopy, onDelete, onEdit, onSaveInline, onToggleMenu, plan }: Props) {
  ...
  return <article className="itinerary-card" data-plan-id={plan.id} draggable onDragStart={(event) => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("application/x-tuyu-itinerary", plan.id); }} style={{ "--type-color": style.color, "--type-tint": style.tint, zIndex: isMenuOpen ? 2 : 1 } as CSSProperties}>
