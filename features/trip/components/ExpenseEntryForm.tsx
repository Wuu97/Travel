...
export function ExpenseEntryForm({ amount, name, occurrence, onAmountChange, onNameChange, onOccurrenceChange, onRelatedItineraryChange, onSave, onTypeChange, plans, relatedItineraryItemId, type }: Props) {
  return (
    <div className="expense-form">
      <input placeholder="消费名称" value={name} onChange={(event) => onNameChange(event.target.value)} />
      <input type="number" placeholder="金额" value={amount} onChange={(event) => onAmountChange(event.target.value)} />
