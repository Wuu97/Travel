import { normalizeTripCategory, type ExpenseCategory, type TripExpense } from "./model";
...
export function normalizeTripExpense(value: Record<string, unknown>, fallbackOccurrence: TripExpense["occurrence"]): TripExpense | null {
  ...
  const type = normalizeTripCategory(value.type);
  if (!title.trim() || !type || !Number.isFinite(value.amount) || typeof value.amount !== "number" || value.amount <= 0) return null;
  return createTripExpense({
    id: typeof value.id === "string" && value.id ? value.id : undefined,
    title, type, amount: value.amount, occurrence,
...
export function createTripExpense(input: TripExpenseInput): TripExpense {
  ...
  if (!title || !Number.isFinite(required.amount) || required.amount <= 0 || !normalizeTripCategory(required.type)) throw new TypeError("Invalid trip expense");
  return {
