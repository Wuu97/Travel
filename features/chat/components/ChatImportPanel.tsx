    ...
    <div className="ai-imports">
      ...
      {itineraryItems.length > 0 && (
        <section>
          <header>
            <b>可导入行程</b>
            {selectedItineraries.length > 0 && <button disabled={!canEditTrip} onClick={() => onAddItineraries(selectedItineraries)}>添加已选 {selectedItineraries.length} 项</button>}
          </header>
      ...
      {expenseItems.length > 0 && (
        <section>
          <header>
            ...
            {selectedBudget.length > 0 && <button disabled={!canEditTrip} onClick={() => onAddExpenses(selectedBudget, "budget")}>导入已选 {selectedBudget.length} 项</button>}
          </header>
