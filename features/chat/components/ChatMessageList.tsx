    ...
    <>
      ...
      {messages.map((message, index) => (
        <div key={`${message.role}-${index}`} className={`bubble ${message.role === "user" ? "user-bubble" : "assistant-bubble"}`}>
          {message.role === "assistant" ? <><div className="rich-message"><ReactMarkdown remarkPlugins={[remarkGfm]}>{message.content}</ReactMarkdown></div>{message.verifiedDataUnavailable && <p className="memory-state memory-error" role="status">实时地点数据暂不可用 <button type="button" disabled={busy} onClick={onRetryVerifiedData}>重试实时数据</button></p>}{renderImports(message)}</> : message.content}
        </div>
      ))}
