...
export class AmapRouteProvider implements RouteProvider {
  ...
  async getRoute(input: RouteInput): Promise<TravelRoute | null> {
    ...
    const data = await this.client.request<AmapRouteResponse>(routePaths[mode], { origin: coordinates(input.from), destination: coordinates(input.to), ...(mode === "driving" ? { strategy: 32 } : {}), ...(mode === "cycling" ? { show_fields: "cost" } : {}) });
