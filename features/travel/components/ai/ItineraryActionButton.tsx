...
export function ItineraryActionButton({ action, index, targetName, category, isPlanAdded, onAddItineraries }: Props) {
  ...
  return <button className="rich-card-action" type="button" disabled={added || !canEditTrip} onClick={() => { onAddItineraries([item]); collectFeedbackEvent({ type: "add_to_trip", itemType: action.type === "add_place" ? "place" : action.type === "add_restaurant" ? "restaurant" : "route", itemId: action.targetId, category }); }}>{added ? "✓ 已加入" : "加入行程"}</button>;
}
