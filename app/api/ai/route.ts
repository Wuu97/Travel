...
export async function POST(request: Request) {
  const { message, context, history } = await request.json() as { message?: string; context?: string; history?: ChatMessage[] };
  ...
  if (!message?.trim()) return Response.json({ error: "请输入旅行问题。" }, { status: 400 });
