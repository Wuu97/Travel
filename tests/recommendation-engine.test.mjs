...
const sources = ["features/ai/schemas/context.ts", "features/memory/model.ts", "features/ai/recommendation/model.ts", "features/ai/recommendation/engine.ts", "features/ai/recommendation/index.ts"];
...
test("ranks items that match nature interests above neutral items", async () => {
  ...
  try {
    const { rankTravelItems } = await compilation.importModule("ai/recommendation/engine.js");
    const result = rankTravelItems({ items: [{ id: "mall", name: "购物中心", category: "购物" }, { id: "lake", name: "西湖", category: "自然景观" }], memoryContext: { preferences: { interests: ["nature"] } } });
...
test("lowers shopping items without deleting them and supports relaxed pace", async () => {
  ...
  try {
    const { rankTravelItems } = await compilation.importModule("ai/recommendation/engine.js");
    const result = rankTravelItems({ items: [{ id: "mall", name: "商业街", category: "购物" }, { id: "park", name: "湖滨公园", category: "自然" }], memoryContext: { preferences: { dislikes: ["shopping"], pace: "relaxed" } } });
...
test("keeps provider order unchanged without memory preferences", async () => {
  ...
  try {
    ...
    assert.deepEqual(rankTravelItems({ items }).sortedItems, items);
  } finally { await compilation.cleanup(); }
});
